...
try:
    ...
    print("[WAITER A2A] Error: Could not import MCP tools", file=sys.stderr)
    McpToolset = None
...
if McpToolset:
    ...
    if orders_path is None:
        print(f"[WAITER A2A] ⚠️  Could not find orders_mcp_server.py in any of: {orders_possible_paths}")
    else:
        try:
            f_out = io.StringIO()
            f_err = io.StringIO()
            ...
            with redirect_stdout(f_out), redirect_stderr(f_err):
                ...
                print(f"[WAITER A2A] ✅ Connected to orders MCP server (using {orders_path})")

        except Exception as e:
            print(f"[WAITER A2A] ⚠️  Could not connect to orders MCP: {e}")
